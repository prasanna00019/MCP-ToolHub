...
def find_duplicates(
    ...
    Examples:
        find_duplicates("users", ["email"])
        find_duplicates("products", ["sku", "vendor_id"], return_mode="all_duplicates")
    """
    start_time = time.time()
    warnings = []
    ...
    try:
        ...
        valid_modes = ["summary", "all_duplicates"]
        if return_mode.lower() not in valid_modes:
            return _format_result(
                status="error",
                operation="find_duplicates",
                message=f"Invalid return_mode '{return_mode}'. Must be one of: {', '.join(valid_modes)}"
            )
        ...
        group_columns = columns + ["duplicate_count"]
        ...
        duplicate_summary = [dict(zip(group_columns, row)) for row in duplicate_groups]
        total_duplicate_rows = sum(row["duplicate_count"] - 1 for row in duplicate_summary)
        
        result = {
            "table": table_name,
            "columns_checked": columns,
            "duplicate_groups": len(duplicate_groups),
            "total_duplicates": total_duplicate_rows,
            "duplicate_summary": duplicate_summary
        }
        
        # If requested, fetch all duplicate rows
        if return_mode.lower() == "all_duplicates":
            all_duplicates = []
            for group in duplicate_groups:
                conditions = [f"{col} = %s" for col in columns]
                where_clause = " AND ".join(conditions)
                values = list(group[:-1])  # Exclude count column
                ...
                duplicate_rows = [dict(zip(cols, row)) for row in rows]
            ...
            result["all_duplicates"] = all_duplicates
        ...
        return _format_result(
            status="success",
            operation="find_duplicates",
            rows_affected=total_duplicate_rows,
            duration_ms=duration_ms,
            result=result,
            message=f"Found {len(duplicate_groups)} duplicate groups ({total_duplicate_rows} duplicate rows)",
            warnings=warnings
        )
...
def validate_foreign_keys(table_name: Optional[str] = None) -> Dict:
    """
    ...
    try:
        ...
        if not foreign_keys:
            duration_ms = (time.time() - start_time) * 1000
            message = f"No foreign keys found" + (f" for table '{table_name}'" if table_name else "")
            return _format_result(
                status="success",
                operation="validate_foreign_keys",
                duration_ms=duration_ms,
                result={
                    "tables_checked": 0,
                ...
                message=message,
                warnings=["No foreign key constraints to validate"]
            )
        ...
        for fk in foreign_keys:
            ...
            cursor.execute(f"""
                SELECT COUNT(*) as orphaned_count,
                       ARRAY_AGG(DISTINCT t.{column}) FILTER (WHERE t.{column} IS NOT NULL) as orphaned_values
                FROM {table} t
            ...
            orphaned_values = result[1] if result and result[1] else []
            ...
            if orphaned_count > 0:
                violations.append({
                    ...
                    "sample_orphaned_values": orphaned_values[:10],  # Limit to 10 samples
                    "fix_sql": f"DELETE FROM {table} WHERE {column} NOT IN (SELECT {ref_column} FROM {ref_table});"
                })
        
        duration_ms = (time.time() - start_time) * 1000
        
        status = "success" if not violations else "warning"
        message = f"Validated {len(foreign_keys)} foreign keys, found {len(violations)} violations"
        
        if violations:
            warnings.append(f"Found {len(violations)} foreign key violations that need attention")
        
        return _format_result(
            status=status,
            operation="validate_foreign_keys",
            rows_affected=sum(v["orphaned_records"] for v in violations),
            duration_ms=duration_ms,
            result={
                "tables_checked": len(set(fk[0] for fk in foreign_keys)),
                "foreign_keys_validated": len(foreign_keys),
                "violations": violations,
                "total_orphaned_records": sum(v["orphaned_records"] for v in violations)
            },
            message=message,
            warnings=warnings
        )
        ...
        return _format_result(
            ...
            operation="validate_foreign_keys",
            duration_ms=duration_ms,
            message=f"Failed to validate foreign keys: {str(e)}"
        )
